...
metadata:
  name: os-ca-cert-destination-rule
spec:
  host: "some-external-site.com"
  trafficPolicy:
    tls:
...
func setupEcho(t *testing.T, ctx resource.Context, mode TrafficPolicy) (echo.Instance, echo.Instance) {
	...
	if err := ctx.Config().ApplyYAML(serviceNamespace.Name(), DestinationRule); err != nil {
		t.Errorf("failed to apply destination rule: %v", err)
	}
